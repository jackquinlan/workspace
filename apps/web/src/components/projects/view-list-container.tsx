...
export function ViewListContainer({ project, views }: ViewListContainerProps) {
    ...
    return (
        <div className="flex items-center justify-between gap-0.5 h-10 border-b">
            <div ref={containerRef} className="flex items-center gap-2 w-2/3">
                <ol ref={viewsListRef} className="flex list-none items-center gap-2">
                    <div className={cn("flex h-10 items-center", !params.viewId && "border-primary border-b-2")}>
                        <Link 
                            className="hover:bg-accent flex h-7 px-1 w-fit cursor-pointer items-center gap-1 truncate rounded-md p-1 text-sm"
                            href={`/p/${project.id}`} 
                        >
