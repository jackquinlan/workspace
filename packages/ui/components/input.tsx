...
const inputClass =
    "flex h-7 w-full rounded-md bg-transparent px-2 py-1 text-sm transition-colors file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50";
        ...
        if (addPrefix || addSuffix) {
            return (
                <div className="border-input flex h-7 w-full items-center space-x-1 rounded-md border text-sm">
                    ...
                    <input
                        type={type}
                        className={cn(inputClass, "h-7 border-none outline-none", className)}
                        ref={ref}
                        {...props}
                    />
                    {addSuffix && (
