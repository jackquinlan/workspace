...
const MultiTagInput = React.forwardRef<
    HTMLDivElement, 
    MultiTagSelectProps
>(({ tags, onChange, ...props }, ref) => {
    ...
    return (
        <div ref={ref} className="flex flex-wrap rounded-md border bg-background h-24">
            {tags.length > 0 && (
            ...
            <Input ref={inputRef} className="border-0 shadow-none" onKeyDown={handleKeyPress} {...props} />
        </div>
    );
});
